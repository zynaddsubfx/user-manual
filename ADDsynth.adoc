...
- *Sync:* if this option enabled the lfo oscillates in phase for all notes
...
===== Phase
The value of the modulator is applied as phase offset. That's what Yamaha calls FM.

===== FM
The value of the modulator is applied as frequency offset. Because of the wide applicable range it is nearly impossible to prevent aliasing at high amounts.
...
The AA button enables the anti aliasing sampling filter. Can be usfull when using high range frequency LFOs or FM.
...
Attention: When the frequency is modulated the resonance spectrum is shifted, too.

In the diagram the spectral fingerprint can be drawn by hand or generated by random generators with fine, medium or large granularity.
The diagram can also be smoothed. The resulting diagram can be shifted or scaled with respect to the full spectrum.

=== signal routing diagram
image::imgs/flow_add_voice.png[]

simplified signal routing for a single voice
