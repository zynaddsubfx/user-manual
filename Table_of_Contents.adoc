...
=== _Table of Contents for the Writers_

NOTE:: This document is intended as an armature for the Zyn-Fusion user manual writers to fill in.
...
Zyn-Fusion is a powerful, modern, multiplatform, open-source software sound synthesizer. Zyn-Fusion is a successor to ZynAddSubFX, bringing in a new graphical user interface, and improved backend. It can work as a standalone application, or as a VST, LV2 or DSSI plugin. ==anything else¿==

=== Main features

NOTE:: TODO Maybe it's not a good idea to repeat the feature list from the main page¿ Or maybe it's a good idea to try to update that instea
...
NOTE:: TODO describe app-wide patterns that the user should know to work more efficiently

There are a few types of controls:
...
The side bar is used to navigate between different parts of the user interface.
...
Zyn-Fusion is no different. Let's talk about various types of modules that you'll encounter exploring Zyn-Fusion. If you take some time to familiarize yourself with these - you'll have a much easier time finding your way through the interface for each of the synthesizing engines found in Zyn-Fusion.
...
NOTE:: TODO ==everything about assigning and using macros==
