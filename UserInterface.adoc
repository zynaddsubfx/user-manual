...
MDW.D::
    Sets the MIDI CC modwheel depth.

FC.D::
    Sets the MIDI CC filter cutoff frequency.

FQ.D::
    Sets the MIDI CC filter resonance.

BW.D::
    Sets the MIDI CC bandwidth depth.

PCH.D::
    Sets the MIDI pitch wheel depth, expressed in semitones (Of course, 100% of a semitone is one semitone, 200% is two, etc).
    By default, it's set to bend by two semitones.

PAN.D::
    Sets the MIDI panning depth.

MDW.EXP::
    Toggles the modwheel exponential mode.

EXP.RCV::
    Toggles expression MIDI recieve.

FMA.RCV::
    Toggles FM amplitude MIDI recieve.

BW.EXP::
    Toggles bandwidth exponential mode.

VOL.RCV::
    Toggles volume MIDI recieve.

SUS.RCV::
    Toggles sustain MIDI recieve.

////
TODO

- What is the auto mode?
- Everything here is susceptible to change, since https://zynaddsubfx.sourceforge.io/Doc/#_portamento
  says the portamento distance is about frequencies and not notes, and that's apparently useful for microtonal.
  I'll work the basic idea out first, though.
////
===== Portamento

Portamento is a term that describes a pitch-slide from one note to another.
Protamento can be useful whenever you want a smooth transition between two notes,
instead of one stopping and another one playing.

PRT.CRV::
    Toggles if ZynAddSubFX will recieve portamento on/off requests from an external MIDI device.

AUTO::
    TODO

ENABLE::
    Toggles if portamento is on/off.

>/< buttons::
    If set to `>`, portamento will occur when the distance between the two keys is greater than the THRESH value.
    `<` does the same, but for distances smaller than THRESH.
    By default, it's set to `>`.

THRESH number field::
    The numeric value which triggers the portamento.
    The exact behaviour is defined by the `>` / `<` buttons, described above.

PROP.::
    Proportional portamento. TODO

// What's the relation between the value and actual time (if there is any, maybe it's bound to frequency)?
TIME::
    Time for which the portamento slides up/down.
...
// TODO: Preset/Patch. Use preset, it's what's in the UI.
// TODO: If not fixed, mention how enabling/disabling a bank shows all the patches.
// TODO: Image
